...
import __changelogTag from './tags/changelog.js';
...
class DocblockBlock {
  ...
  public static registerTag(tagName: string, parser: any): void {
    ...
    if (typeof parser !== 'function')
      throw new Error(
        `The "<yellow>parser</yellow>" parameter of the static "<cyan>DocblockBlock</cyan>" class method needs to be a "<green>Function</green>"`,
      );
    // register the tag
    ...
    return new Promise(async (resolve) => {
      ...
      for (let i = 0; i < Object.keys(docblockObj).length; i++) {
        ...
        if (this.settings.tags[prop] && prop !== 'src') {
          let res;
          try {
            res = await this.settings.tags[prop](value, this.settings);
          } catch (e) {
            console.error(
              // @ts-ignore
...
DocblockBlock.registerTag('changelog', __changelogTag);
