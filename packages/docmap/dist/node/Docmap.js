...
import { checkPathWithMultipleExtensions, ensureDirSync, fileName, folderPath, readJsonSync, writeFileSync, writeJsonSync, } from '@blackbyte/sugar/fs';
import { encodeEntities } from '@blackbyte/sugar/html';
import { filterDeep, get, mapDeep, mergeDeep, set, sort, sortDeep, } from '@blackbyte/sugar/object';
...
class Docmap {
    ...
    build(params) {
        ...
        return new Promise((resolve) => __awaiter(this, void 0, void 0, function* () {
            ...
            for (let i = 0; i < filesInPackage.length; i++) {
                ...
                console.log(`<yellow>[build]</yellow> Parsing file "<cyan>${__path.relative(packageRootDir(), 
                // @ts-ignore
                filePath)}</cyan>"`);
                ...
                for (let j = 0; j < docblocks.length; j++) {
                    ...
                    if (docblock.namespace && !this._entries[dotPath]) {
                        docblockObj = Object.assign(Object.assign({}, docblockEntryObj), { filename, extension: filename.split('.').slice(1)[0], relPath: __path.relative(packageRootDir(), filePath) });
            ...
            if (finalParams.save) {
                ...
                if (finalParams.outDir || typeof finalParams.outPath === 'function') {
                    for (let [namespace, docmapObj] of Object.entries(docmapJson.generated.map)) {
                        ...
                        if (finalParams.json) {
                            ...
                            console.log(`<green>[save]</green> JSON file saved <green>successfully</green> under "<cyan>${outPath.replace(packageRootDir() + '/', '')}</cyan>"`);
                        }
                        // mdx
                        if (finalParams.mdx) {
                            // update outpath
                            ...
                            console.log(`<green>[save]</green> MDX file saved <green>successfully</green> under "<cyan>${mdxOutPath.replace(packageRootDir() + '/', '')}</cyan>"`);
                        }
                    }
                }
                ...
                if (typeof finalParams.outPath === 'string') {
                    fs.writeFileSync(finalParams.outPath, JSON.stringify(docmapJson, null, 4));
                    console.log(`<green>[save]</green> docmap.json file saved <green>successfully</green> under "<cyan>${finalParams.outPath.replace(packageRootDir() + '/', '')}</cyan>"`);
                }
            }
            resolve(docmapJson);
        }));
    ...
    toMdx(docmapObj) {
        ...
        if (docmapObj.since) {
            result.push(`<div class="docmap_since"><span class="docmap_since-label">Since:</span><span class="docmap_since-value">${docmapObj.since}</span></div>`);
        }
        if (docmapObj.platform) {
            result.push(`<div class="docmap_platform"><span class="docmap_platform-label">Platform:</span>${docmapObj.platform
                .map((p) => `<span class="docmap_platform-value -${p.name}">${p.name}</span>`)
                .join('')}</div>`);
        }
        if (docmapObj.status || docmapObj.since || docmapObj.platform) {
            result.push('</div>');
        ...
        if (docmapObj.todo) {
            ...
            docmapObj.todo.forEach((todo) => {
                result.push('<li class="docmap_item">');
                result.push(`<span class="docmap_description">${todo.description}</span>`);
                if (todo.priority) {
                    result.push(`<span class="docmap_priority -${todo.priority}">${todo.priority}</span>`);
                }
                result.push('</li>');
            });
            result.push('</ul>');
        ...
        if (docmapObj.author) {
            ...
            if (docmapObj.author.email) {
                result.push(`<span class="docmap_email">${docmapObj.author.email}</span>`);
            }
            if (docmapObj.author.url) {
                result.push(`<a href="${docmapObj.author.url}" target="_blank" class="docmap_url">${docmapObj.author.url}</a>`);
            }
            result.push('</li>');
            result.push('</ul>');
            result.push('</div>');
        ...
        if (docmapObj.contributor) {
            ...
            docmapObj.contributor.forEach((contributorObj) => {
                ...
                if (contributorObj.email) {
                    result.push(`<span class="docmap_email">${contributorObj.email}</span>`);
                }
